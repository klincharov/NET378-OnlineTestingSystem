...
//TODO fix spacing and new lines before comments/javadoc!!!
//TODO check style ! { new lines etc.
...
public class Client extends JFrame implements ActionListener {
    ...
    /**
     * Array used for book-marking the questions, which are to be revised later.
     */
    int[]       bookmarkArray;
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        if (evt.getSource() == nextBut) {
            try {
                ...
                displayMessage("Unkown error! " + ex.getMessage());
            }
        ...
        if (evt.getActionCommand().equals("Results")) {    // !!! Results != Result
            current++;
            System.out.println("Correct " + count + " out of " + questions.length);
    ...
    void set() {
        try {
            ...
            if (current < questions.length) {
                ...
                final int MODIFIER = current * 4;

                // loop and place every pair of 4 answers
                for (int k = 0; k < 4; k++) {
                    jButtons[k].setText(answers[k + MODIFIER]);
                }
